...
app.post("/signup", async (req: Request, res: Response) => {
	...
	const { userName, password, profilePicture, firstName, lastName } =
		parsed.data;
...
app.post("/create-room", verifyUser, async (req: Request, res: Response) => {
	...
	try {
		...
		if (e.code && e.code === "P2002") {
			res.status(401).json({
				error: "Room already exists, please try another name",
			});
			return;
		}
		res.json({ error: e });
...
app.get("/shape/:roomId", verifyUser, async (req: Request, res: Response) => {
	...
	console.log(
		`Request url = ${req.url}, method = ${req.method}, hostName = ${req.hostname}`,
	);
	if (!roomId) {
...
app.get(
	...
	async (req: Request, res: Response) => {
		...
		try {
			...
			res.json({
				shapes,
				message: "Token generated and shapes fetched",
				token,
			});
		} catch (e: any) {
			res.json({ error: e.toString() });
